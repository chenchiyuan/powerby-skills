...
### 2.3 Gate 2: 澄清完整性检查（P2→P3）

**对应阶段**: P2 - 需求澄清
**检查时机**: `/powerby.clarify` 指令完成后
**检查目标**: 确保需求澄清充分且完整
...
**文档一致性验证**
- [ ] 所有澄清已同步回prd.md对应章节
- [ ] 所有澄清已同步回function-points.md
- [ ] 澄清记录格式正确、内容完整
- [ ] 文档路径符合规范

**核心类别清晰度达标**
- [ ] 覆盖度状态: 核心类别≥80%为"Clear"
- [ ] 核心类别包括：功能范围、数据模型、交互流程、非功能属性
- [ ] Needs Work类别数量 ≤ 20%
- [ ] Unknown类别数量 ≤ 5%
...
```bash
# 检查文件存在
ls -la docs/iterations/*/clarifications.md

# 检查覆盖度分析
grep -c "Clear" docs/iterations/*/clarifications.md
grep -c "Needs Work" docs/iterations/*/clarifications.md
grep -c "Unknown" docs/iterations/*/clarifications.md

# 检查澄清问题数量
grep -c "问题" docs/iterations/*/clarifications.md
```
...
### 2.9 Gate 8: 交付验收检查（P8→完成）

**对应阶段**: P8 - 交付
**检查时机**: `/powerby.review` 指令P8阶段完成后
**检查目标**: 确保项目交付完整且成功
