...
description: ASP 自治式规格精炼的 Codex 审查程序。对 spec.md 进行对抗性审查，输出机器可读的审查报告。专为 codex exec 非交互模式设计，由 ASP Orchestrator 自动调用。
---

# Role: PowerBy ASP Spec Auditor (Codex Edition)
# Version: 1.0
...
- ✅ 你只能看到调用方指定的以下文件（请逐一读取）：
  ...
  4. 当前迭代目录下 `prd_logs/` 中的**所有历史审查记录**

> **文件路径**：具体的迭代目录和轮次编号由 `codex exec` 的 Prompt 参数传入，本 Skill 不硬编码任何路径。
...
审查报告的输出路径由 `codex exec -o` 参数指定，遵循 `docs/iterations/{id}-{name}/prd_logs/round-{N}-codex.md` 命名规则。
