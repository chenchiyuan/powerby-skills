...
description: |
  当评审流程已经有 feature_spec_registry，并需要还原 Feature->Feature、Feature->External、Feature->Data Object 的依赖关系，产出 dependency_registry 与依赖矩阵时使用。
  它负责描述能力之间如何衔接，不负责发明不存在的功能。不适用于 gap 严重度判断。
compatibility:
  - python3
  - local-filesystem
---
...
- 输出 dependency_registry，每条依赖带 dependency_type、evidence_refs、confidence。
- 依赖覆盖 feature、external_system、data_object 三类目标。
- 判断不足时返回 partial，不用本地规则表硬补。
- 生成 `09-dependency-matrix.md`。

## Strategy

### 设计哲学

1. **显式证据优先** -- 优先采用文档、测试、命令编排和服务调用的显式证据。
2. **保守推断** -- 只有上下文高度一致时才建立 inferred 依赖，并保留置信度。
3. **缺失优于错误** -- 缺失依赖比错误依赖更可接受。
4. **三类目标** -- 依赖目标必须可回溯到 function_id、external_id 或 data_object_id。
5. **渲染下沉** -- 依赖矩阵 Markdown 交给 renderer script。

## Tools and capability boundaries

- 读取 `context.feature_spec_registry`、`context.current_facts`、`context.evidence_registry`。
- 允许从规格卡候选事实、代码入口、测试链路中判断依赖。
- 使用 `scripts/render_dependency_deliverable.py` 落盘。
- 不修改产品对象，不做最终差异裁定。
- 不用本地硬编码枚举替代依赖判断。
...
- `scripts/render_dependency_deliverable.py` -- 渲染阶段执行
