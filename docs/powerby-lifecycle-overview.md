...
**更新说明**:
- v2.2.0 - 更新P0+P1合并优化，增加快速流程定义，修复门禁系统不一致问题
...
5. [阶段详细说明](#5-阶段详细说明)
6. [指令流程设计](#6-指令流程设计)
7. [Skills与阶段映射](#7-skills与阶段映射)
8. [原子技能复用模式](#8-原子技能复用模式)
9. [操作流程指引](#9-操作流程指引)
10. [质量门禁机制](#10-质量门禁机制)
...
PowerBy提供两种开发流程，以适应不同规模和复杂度的项目需求：

| 流程类型 | 适用场景 | 时间周期 | 阶段范围 | 特点 |
|---------|---------|---------|---------|------|
| **标准流程** | 完整功能开发 | 2-4周 | P0-P8 | 完整流程，详细文档，全面质量门禁 |
| **快速流程** | 小需求/增量开发 | ≤5小时 | P0-P5 | 流程精简，自动初始化，人工确认点 |

### 2.2 标准流程 (P0-P8)

#### 2.2.1 流程说明

标准流程遵循完整的P0-P8生命周期，适用于：
- 全新系统设计
- 复杂跨域需求
- 大型功能开发
- 需要详细文档和完整质量把控的项目
...
```
P0: 项目初始化 → P1: 需求定义 → P2: 需求澄清 → P3: 技术调研 → P4: 架构设计 → P5: 任务规划 → P6: 开发实现 → P7: 质量审查 → P8: 发布交付
```
...
**优化前**:
```
P0: /powerby.initialize → P1: /powerby.define → P2: /powerby.clarify
```

**优化后**:
```
P0+P1: /powerby.define (自动初始化 + 需求定义+澄清)
```

**核心改进**:
1. **自动初始化**: 检查项目文件，不存在时自动创建
2. **需求定稿制**: P1阶段融合需求定义与澄清，同步推理减少冗余
3. **减少对话轮次**: 避免"P1完成→等待确认→进入P2"的循环

#### 2.3.3 快速流程阶段

```
P0+P1: /powerby.define (自动初始化 + 需求定稿制) → P3: /powerby.quick → P4: /powerby.quick → P5: /powerby.quick
```

**质量门禁 (Q-Gate 0-5)**:
- Q-Gate 0: 自动初始化完成 (P0→P1)
- Q-Gate 1: 需求定稿制完成 (P1→P3)
- Q-Gate 3: 技术调研完成 (P3→P4)
- Q-Gate 4: 架构设计完成 (P4→P5)
- Q-Gate 5: 任务规划完成 (P5→交付)
...
所有涉及P0阶段的命令都会自动检查：
1. `.powerby/project.json` 是否存在
2. `docs/constitution.md` 是否存在

#### 2.5.2 自动创建

如果文件不存在，系统会自动：
1. 创建 `.powerby/project.json`（项目元数据）
2. 创建 `.powerby/iterations.json`（迭代追踪）
3. 创建 `docs/constitution.md`（从模板生成）
