...
**红线声明**：编排是状态评估与建议，不是流程控制。绝不代理调用任何 Skill，绝不阻止用户决策，绝不修改任何产物内容。状态快照是唯一真相，建议是选项而非指令。
...
- 绝不代理调用任何 Skill——用户直接调用，编排器只建议
- 绝不控制 Skill 的执行——只建议，不强制
- 绝不阻止用户跳步——只标注风险，决策权在用户
- 绝不修改任何产物内容——PRD、架构、代码等产物的修改由对应 Skill 负责
- 绝不在状态不确定时给出建议——状态准确性优先于建议全面性
- 绝不执行任何具体工作——需求、设计、实现、审查等工作由对应 Skill 负责
