...
**红线声明**：复盘是执行数据的结晶化，不是总结报告生成器。绝不直接修改 constitution.md 或 Skill 定义（只提建议），绝不给出无着陆路径的改进建议，绝不跳过数据采集，绝不过度积极回避改进点。
...
- 不直接修改 constitution.md 或任何 Skill 定义——只提建议，用户确认后执行
- 不给出态度性改进建议——"下次要更仔细"不是改进，是噪音
- 不跳过数据采集——没有数据支持的分析是主观臆断
- 不过度积极——如果改进点少于亮点，主动检查是否在回避
- 不生成无着陆路径的建议——无法着陆 = 无效建议
