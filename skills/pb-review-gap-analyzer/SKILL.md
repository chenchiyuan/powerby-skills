...
description: |
  当对象、关系和冲突已经齐备，并需要识别需求与实现的差异、缺失对象、链路断点与残留冲突时使用。
  它必须只基于显式证据输出 gap 和 difference，不能脑补问题。不适用于提出修复方案或编写最终报告。
compatibility:
  - python3
  - local-filesystem
---
...
- 区分文档有代码无、代码有文档无、对象孤立、链路断点，以及测试化缺口。
- 结论都能回溯到现有 registry 或证据 ID。
- 缺少必需 registry 时停止，不凭空分析。
- 输出 difference_registry 并生成 `06-gap-analysis.md`。

## Strategy

### 设计哲学

1. **显式差异先行** -- 先看显式差异，再看结构性缺口。
2. **交叉验证** -- 用 feature state、relation coverage、testability_status 与 conflict registry 交叉验证。
3. **证据链闭环** -- 对任何"看起来不对"的地方，先找证据链再下结论。
4. **不确定即保留** -- 没证据时保留"不确定"，不制造 alarm。
5. **发现而非方案** -- 输出的是"评审发现"，不是"整改方案"。

## Tools and capability boundaries

- 读取 `context.object_registry`、`context.feature_state_registry`、`context.relation_registry`、`context.conflict_registry`。
- 可做集合对比、状态比对、覆盖率异常检测。
- 使用 `scripts/render_gap_analysis.py` 负责交付物落盘。
- 不新增对象，不重写上游关系。
- 不负责生成最终报告文本。
- 不允许通过脚本把分析外包到后端模型。
...
- gap 需显式标注 gap_severity，引用共享严重程度规则。
- 上游 registry 缺失时优先停止并指出缺失环节。
...
5. 调用 `scripts/render_gap_analysis.py` 生成 `06-gap-analysis.md`。
...
- `../pb-review/schemas/testability-status-rules.md` -- 测试化缺口判断时加载
- `../pb-review/schemas/gap-severity-rules.md` -- 严重度标注时加载
- `scripts/render_gap_analysis.py` -- 渲染阶段执行
