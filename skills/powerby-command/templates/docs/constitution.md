# 项目宪章 (Project Constitution)

**创建时间**: {{TIMESTAMP}}
**项目名称**: {{PROJECT_NAME}}
**版本**: v1.0.0

---

## 一、核心理念

### 1.1 基本信念

**零假设原则 (Zero-Assumption Principle)**：
- 绝不猜测用户的模糊意图
- 当指令不明确或缺少关键信息时，提出具体问题澄清需求
- 优先确认而非假设

**Mixin思维 (Mixin Thinking)**：
- 采用增量补充、组合优于完整的理念
- 各角色以Mixin方式共同维护单一功能文档
- 随着项目推进逐步丰富，不追求一次性完美
- 容忍阶段性缺失，文档像代码一样迭代演进

**小步提交原则**：
- 代码应频繁提交，每次提交都需确保可编译且通过测试
- 优于"大爆炸"式开发
- 增量式地提交可工作的代码

**务实原则**：
- 借鉴现有代码，而后创造
- 拥抱务实，而非固守教条
- 灵活适应项目现实，选择最合适的方案

**意图清晰原则**：
- 编写"无聊"且一目了然的代码
- 意图清晰，优于炫技代码

### 1.2 何谓简单

- **单一职责**：每个函数/类的目标专一
- **避免过早抽象**：只在必要时进行抽象
- **拒绝奇技淫巧**：永远选择最直接、最易懂的方案
- **无需解释**：如果一段代码需要注释才能看懂，那它就太复杂了
- **Mixin式增量**：从简单开始，逐步添加

## 二、工作流程

### 2.1 顾问式任务处理流程

从接收一个新任务到开始编码，必须遵循以下顾问式流程：

1. **理解与信息收集**
   - 全面了解现有服务和相关代码
   - 获取执行任务所必需的所有背景信息

2. **确认目标与边界**
   - 提交对任务目标、范围和边界的理解
   - 等待确认，确保对"做什么"和"不做什么"达成共识

3. **创建并提交方案文档**
   - 包含现状分析 (Current State)：使用 Markdown + Mermaid
   - 包含你的思考 (Your Analysis)：分析问题、识别关键挑战
   - 包含方案选项 (Solution Options)：至少两种可行方案
   - 包含你的建议 (Your Recommendation)：明确推荐方案及理由

4. **方案审查与决策**
   - 必须在方案获得明确批准后，才能进入下一步

5. **生成计划文档**
   - 根据选定的方案创建详细的 IMPLEMENTATION_PLAN.md

6. **执行实现**
   - 严格依据计划文档进行开发

### 2.2 实现流程

**理解 (Understand)**：
- 研究代码库中的现有模式

**测试 (Test)**：
- 编写失败的测试（红灯）

**实现 (Implement)**：
- 编写最精简的代码，让测试通过（绿灯）

**重构 (Refactor)**：
- 在测试的保护下，清理和优化代码

**提交 (Commit)**：
- 撰写清晰的提交信息，并关联到开发计划

### 2.3 受阻处理机制 (3次尝试失败后)

**针对同一问题，最多尝试3次。如果依然无法解决，立刻停止。**

然后遵循以下步骤：
1. 记录失败过程
2. 研究替代方案
3. 反思基本问题
4. 尝试不同角度

## 三、技术标准

### 3.1 架构原则

- **SOLID原则**：遵循五大设计原则，构建高内聚、低耦合的模块
- **DRY原则 (Don't Repeat Yourself)**：消除重复，将通用逻辑抽象为单一、权威的实现
- **奥卡姆剃刀原则 (Occam's Razor)**：如无必要，勿增实体。永远选择能解决问题的最简单的方案
- **演进式架构原则 (Evolutionary Architecture)**：支持增量、引导式的架构变更
- **组合优于继承**：优先使用依赖注入
- **接口优于单例**：确保代码的可测试性和灵活性
- **显式优于隐式**：保持数据流和依赖关系的清晰
- **测试驱动开发**：尽可能采用TDD

### 3.2 代码质量

**每次提交都必须**：
- 成功编译
- 通过所有现有测试
- 为新功能添加测试
- 符合项目的格式化/Linting规范

**提交前的检查清单**：
- 运行格式化和Linter工具
- 自我审阅（Self-review）代码变更
- 确保提交信息解释了"为什么"要这么做

### 3.3 错误处理

- 快速失败，并提供有描述性的错误信息
- 包含调试上下文，方便定位问题
- 在合适的层级处理错误
- 绝不静默地"吞掉"异常

## 四、决策与集成

### 4.1 决策框架

当存在多个合理方案时，按以下优先级进行选择：
1. **可测试性**：我能轻松地测试这个方案吗？
2. **可读性**：6个月后，其他人能看懂这段代码吗？
3. **一致性**：这是否符合项目现有的模式？
4. **简单性**：这是能解决问题的最简单的方案吗？
5. **可逆性**：如果未来发现决策错误，修改它的成本有多高？

### 4.2 融入项目

**学习代码库**：
- 找到3个类似的功能/组件进行研究
- 识别并遵循通用的模式和约定
- 尽可能复用项目中已有的库和工具
- 遵循现有的测试模式

**工具使用**：
- 使用项目已有的构建系统
- 使用项目已有的测试框架
- 使用项目已有的格式化/Linter配置
- 在没有充分理由前，不要引入新工具

## 五、质量门禁

### 5.1 "完成"的定义 (Definition of Done)

- [ ] 测试已编写并通过
- [ ] 代码遵循项目约定
- [ ] 无Linter/Formatter警告
- [ ] 提交信息清晰明确
- [ ] 实现与计划相符
- [ ] 没有未关联Issue编号的TODO

### 5.2 测试准则

- 测试行为，而非实现细节
- 尽可能做到一个测试只包含一个断言
- 测试名称应清晰描述测试场景
- 使用项目中已有的测试工具/辅助函数
- 测试必须是确定性的，无随机或不稳定的结果

## 六、重要提醒

### 【绝不】
- 使用 --no-verify 绕过提交钩子
- 禁用测试来代替修复
- 提交无法编译的代码
- 做出假设 —— 用现有代码去验证

### 【务必】
- 增量式地提交可工作的代码
- 随时更新你的计划文档
- 从现有实现中学习
- 在3次失败的尝试后，停止并重新评估

---

**本宪章是约束整个项目的最高法则，所有产品、架构、研发决策都必须遵循此宪章。**

**版本**: v1.0.0
**最后更新**: {{TIMESTAMP}}
**适用范围**: {{PROJECT_NAME}} 全项目
